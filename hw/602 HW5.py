...
def exercise11(n):
    ...
    if n % 3 != 0:
        raise ValueError("n must be divisible by 3 for reshaping into 3 rows.")
...
def exercise13(n):
    ...
    #assume upperbound inclusive
    s=pd.Series(np.random.randint(0,n, size=n),index=date)
    s.cumsum().plot()
    plt.show()
...
def exercise14(words):
    ...
    lengths=words_s.map(lambda word: len(word))
